...
class HOSH5P:
    """
    Modelo Operacional de Transformación de Precipitación en Escorrentía. Hidrología Operativa Síntesis de Hidrograma
    """
    type='PQ Model'
    def __init__(self,pars,Boundaries=[0],InitialConditions=[0,0]):
        self.maxSurfaceStorage=pars[0]
        self.maxSoilStorage=pars[1]
        self.soilSystem=SCSReservoirs(pars=[pars[0],pars[1]])
        ...
        self.SurfaceStorage=np.array([0]*(len(self.Precipitation)+1),dtype='float')
        self.SoilStorage=np.array([0]*(len(self.Precipitation)+1),dtype='float')
        self.NetRainfall=np.array([0]*len(self.Precipitation),dtype='float')
        self.Infiltration=np.array([0]*len(self.Precipitation),dtype='float')
        self.Runoff=np.array([0]*len(self.Precipitation),dtype='float')
